...
import Controller.Shop_management;
import DB.CartItems;
import DB.Orders;
 ...
 */
@WebServlet(name = "ProcessCheckout", urlPatterns = {"/ProcessCheckout"})
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (httpSession.getAttribute("loguser") != null) {
                ...
                if (httpSession.getAttribute("cart") != null) {
                    ...
                    Criteria c1 = s.createCriteria(DB.Paymanetmethod.class);
                    c1.add(Restrictions.eq("method", method));
                    DB.Paymanetmethod paymanetmethod = (DB.Paymanetmethod) c1.uniqueResult();

                    Set<ShopsHasStock> loadshops = carttItems.getStock().getShopsHasStocks();
                    for (ShopsHasStock loadshop : loadshops) {
                        shop = loadshop.getShops();
                    }

                    Stock stk = carttItems.getStock();
                    ...
                    s.close();
                    response.sendRedirect("SendOrder?stid="+stk.getIdstock()+"&sid="+sid+"&qty="+qty+"&paymethod="+paymanetmethod.getIdpaymanetMethod()+"&address="+address);
                }
            } else {
